...
Look.prototype.wrap = function (_config) {
    ...
    var wrap = {
        type: "wrap",
        zIndex: 1000,
        titleBarButtons: ["close", "small", "large"], // 要显示的头部按钮 close/small/large
        style: {
            background: "#fff",
            padding: 10
        },
...
Look.prototype.bindEvent = function () {
    ...
    if (this.buttonBar) {
        $self.find(".look-floor .look-floor-btns .floor-btn").each(function (index) {
            ...
            $(this).click(function (e) {
                e.stopPropagation();
                // 将最新的显示层叠到最高
                // that.maxShow(); 点击的时候会挡住msg类型弹窗，暂且注释
                if (btn.close) {
                    that.close();
                }
    ...
    //  点击的时候会挡住msg类型弹窗，暂且注释
    // $self.find(".look-box").click(function (e) {
    //     e.stopPropagation();
    //     // 将最新的显示层叠到最高
    //     that.maxShow();
    // });
...
Look.prototype.reSize = function () {
    ...
    $self.find(".look-box").css({
        left: to0(this.position.left),
        top: to0(this.position.top),
        right: to0(this.position.right),
        bottom: to0(this.position.bottom)
    }).find(".look-floor").show();
...
Look.prototype.create = {
    ...
    createInput: function (that) {
        ...
        var html = "\n        <p>" + that.content + "</p>\n        <input class=\"look-input\" placeholder=\"" + that.placeholder + "\" value=\"" + that.value + "\" />\n        ";
        return html;
    },
